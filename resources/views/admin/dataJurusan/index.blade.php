            ...
            <div id="editModal"
                ...
                <div class="bg-white rounded-lg w-full max-w-md mx-4 p-6 relative">
                    ...
                    <form id="editJurusanForm" method="POST">
                        ...
                        <div>
                            <label class="block mb-1 text-sm font-medium text-gray-700">Kode Jurusan</label>
                            <input type="text" name="kode_jurusan" id="edit_kode_jurusan"
                                class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500">
                            @error('kode_jurusan')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
            ...
            <div id="deleteModal"
                ...
                <div class="bg-white rounded-lg w-full max-w-md mx-4 p-6 relative">
                    ...
                    <p class="text-gray-700 mb-4">
                        ...
                        <span id="delete_jurusan_name" class="font-semibold"></span>?
                    </p>
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-center space-x-2">
                                <button
                                    onclick="openDetailModal({{ $jurusan->id }}, '{{ $jurusan->kode_jurusan }}', '{{ $jurusan->nama_jurusan }}')"
                                    class="bg-blue-500 hover:bg-blue-600 text-white text-sm px-3 py-1.5 rounded">Lihat</button>
                                <button
                                    onclick="openEditModal({{ $jurusan->id }}, '{{ $jurusan->kode_jurusan }}', '{{ $jurusan->nama_jurusan }}')"
                                    class="bg-yellow-500 hover:bg-yellow-600 text-white text-sm px-3 py-1.5 rounded">Edit</button>
                                <button onclick="openDeleteModal({{ $jurusan->id }}, '{{ $jurusan->nama_jurusan }}')"
                                    class="bg-red-600 hover:bg-red-700 text-white text-sm px-3 py-1.5 rounded">Hapus</button>
                            </td>
                        </tr>
                        @empty
                        <tr>
...
<script>
    ...
    function openEditModal(id, kode, nama) {
        ...
        form.action = `{{ route('admin.dataJurusan.update', ':id') }}`.replace(':id', id);
    ...
    function openDeleteModal(id, nama) {
        const form = document.getElementById('deleteJurusanForm');
        form.action = `{{ route('admin.dataJurusan.destroy', ':id') }}`.replace(':id', id);

        document.getElementById('delete_jurusan_name').textContent = nama;
        document.getElementById('deleteModal').classList.remove('hidden');
    }
